...
CREATE TABLE R2D2_ARTURITO.PROVINCIA (
    id_provincia SMALLINT PRIMARY KEY IDENTITY(1,1),
...
CREATE TABLE R2D2_ARTURITO.ESTADO_FISCAL (
    id_estadoFiscal INT PRIMARY KEY IDENTITY(1,1),
...
CREATE TABLE R2D2_ARTURITO.PROMOCION (
    ...
    regla_promocion INT NOT NULL,
    FOREIGN KEY (regla_promocion) REFERENCES R2D2_ARTURITO.REGLA_PROMOCION(id_regla)
);
GO
...
BEGIN
    PRINT 'PROVINCIA'
    INSERT INTO GD1C2024.R2D2_ARTURITO.PROVINCIA(nombre)
    SELECT super_provincia AS nombre
    FROM GD1C2024.gd_esquema.Maestra
    WHERE super_provincia IS NOT NULL
    UNION
    SELECT sucursal_provincia AS nombre
    FROM GD1C2024.gd_esquema.Maestra
    WHERE sucursal_provincia IS NOT NULL
    UNION
    SELECT cliente_provincia AS nombre
    FROM GD1C2024.gd_esquema.Maestra
...
BEGIN
    PRINT 'CATEGORIA'
    --INSERT INTO GD1C2024.R2D2_ARTURITO.CATEGORIA(id_categoria , descripcion)    
	INSERT INTO GD1C2024.R2D2_ARTURITO.CATEGORIA(descripcion)

    SELECT DISTINCT 
		--substring(prod_categoria,13,7),
...
BEGIN
    PRINT 'MARCA'
    --INSERT INTO GD1C2024.R2D2_ARTURITO.MARCA(id_marca, descripcion)    
...
VENTA (Depende de SUCURSAL, EMPLEADO y CAJA)
PAGO (Depende de VENTA, MEDIO_PAGO y DETALLE_TARJETA)
*/
...
--Insertar en Tablas de Detalle:

/*ITEM_VENTA (Depende de VENTA y PRODUCTO)
DETALLE_TARJETA (Depende de TARJETA y CLIENTE)
DESCUENTO_X_PAGO (Depende de PAGO y DESCUENTO)
PROMOCION_X_PRODUCTO (Depende de PROMOCION y PRODUCTO)
MARCA_X_PRODUCTO (Depende de PRODUCTO y MARCA)
SUBCATEGORIA_X_PRODUCTO (Depende de PRODUCTO y SUBCATEGORIA)
PROMOCION_APLICADA (Depende de PROMOCION y ITEM_VENTA)*/
